...
def test_unfilled_limit_entry_is_canceled_without_market_fallback():
    ...
    assert exchange.cancellations == [("limit-buy-1", "ETH/USD")]
    assert exchange.market_buys == []


def test_uncertain_limit_entry_status_is_canceled_without_market_fallback():
    helpers = load_order_helpers()
...
def test_limit_create_failure_can_use_market_entry_fallback():
    helpers = load_order_helpers()
    ...
    assert exchange.market_buys == [("ETH/USD", 200.0)]


def test_unfilled_limit_exit_is_canceled_without_market_fallback():
    helpers = load_order_helpers()
    exchange = FakeExchange(order_status={"status": "open"})
    ...
    assert executed is False
    assert exchange.cancellations == [("limit-sell-1", "ETH/USD")]
    assert exchange.market_sells == []


def test_uncertain_limit_exit_status_is_canceled_without_market_fallback():
    helpers = load_order_helpers()
    exchange = FakeExchange(fetch_order_error=RuntimeError("status unavailable"))
...
def test_position_size_cost_matches_recorded_base_amount():
    helpers = load_order_helpers()
