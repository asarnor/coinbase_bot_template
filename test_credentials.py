...
def main() -> int:
    ...
    if not api_secret:
        ...
        print("✅ COINBASE_API_SECRET is set")
    ...
    try:
        ...
        num_ccy = len([k for k, v in balance.items() if isinstance(v, dict) and v.get("total", 0) > 0])
        print(f"   Found {num_ccy} currencies with balance")
        print()
