...
def extract_filled_amount(order: Dict, default: float = 0.0) -> float:
    if not order:
        return default
    ...
    if filled is None:
        ...
        if amount is not None and remaining is not None:
            try:
                filled = float(amount) - float(remaining)
            except (TypeError, ValueError):
                filled = None

    try:
        filled_amount = float(filled) if filled is not None else float(default)
    except (TypeError, ValueError):
        filled_amount = float(default)
    return max(filled_amount, 0.0)


def cancel_limit_order(exchange, order: Dict, symbol: str, base_currency: str, reason: str) -> bool:
    order_id = order.get("id") if order else None
    if not order_id:
        print(f"[{base_currency}] ⚠️  Could not cancel {reason} limit order: missing order id")
        return False
...
def apply_exit_fill(position: Dict, filled_amount: float) -> Tuple[float, bool]:
    current_amount = position["position_amount"]
    exit_amount = min(max(filled_amount, 0.0), current_amount)
    if exit_amount <= 0:
        return 0.0, False

    remaining_amount = max(current_amount - exit_amount, 0.0)
    fully_exited = remaining_amount <= max(current_amount * 1e-8, 1e-12)
    if fully_exited:
        reset_position_state(position, record_exit=True)
    else:
        position["position_amount"] = remaining_amount
    return exit_amount, fully_exited
...
def place_entry_order(
    ...
    if use_limit_orders:
        ...
        try:
            ...
            print(f"[{base_currency}] ❌ Limit entry status check failed: {exc}")
            cancel_limit_order(exchange, order, symbol, base_currency, "entry")
            return False, 0.0
        ...
        cancel_limit_order(exchange, order, symbol, base_currency, "entry")
        if filled_amount > 0:
            print(f"[{base_currency}] ✅ Limit order partially filled: {filled_amount:.6f} {base_currency}")
            return True, filled_amount
        print(f"[{base_currency}] ⏳ Limit order unfilled; canceled before the next cycle")
        return False, 0.0
...
def place_exit_order(
    ...
    if use_limit_orders and not force_market:
        ...
        try:
            ...
            print(f"[{base_currency}] ❌ Limit exit status check failed: {exc}")
        ...
        cancel_limit_order(exchange, order, symbol, base_currency, "exit")
        if filled_amount > 0:
            print(f"[{base_currency}] ✅ Limit exit partially filled: {filled_amount:.6f} {base_currency}")
            return True, filled_amount
        print(f"[{base_currency}] ⏳ Exit limit order unfilled; canceled before the next cycle")
        return False, 0.0
        ...
        try:
            ...
            if not pos["in_position"]:
                ...
                if price_above_ema and rsi_strong and trend_strong_enough and ema_trending_up and volume_adequate:
                    ...
                    if entry_executed:
                        entry_amount = filled_amount if filled_amount > 0 else amount
                ...
                if (
                    ...
                    exit_executed, filled_amount = place_exit_order(
                        exchange,
                        symbol,
                        base_currency,
                        pos["position_amount"],
                    ...
                    if exit_executed:
                        exit_amount, fully_exited = apply_exit_fill(pos, filled_amount)
                        journal.log_event(
                            "exit_executed",
                            symbol=symbol,
                            profile=profile_name,
