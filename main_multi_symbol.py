    ...
    if use_limit_orders:
        ...
        try:
            ...
            if order_status.get("status") == "closed":
                ...
                return True
            print(f"[{base_currency}] ⏳ Limit order still open; canceling to avoid untracked exposure")
            cancel_unfilled_limit_order(order_id, "entry")
            return False
        except Exception as exc:
    ...
    if use_limit_orders and not force_market:
        ...
        try:
            ...
            if order_status.get("status") == "closed":
                ...
                return True
            print(f"[{base_currency}] ⏳ Exit limit order still open; canceling before the next exit attempt")
            cancel_unfilled_limit_order(order_id, "exit")
            return False
        except Exception as exc:
