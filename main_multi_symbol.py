...
def get_confirmed_filled_amount(order: Dict, fallback_amount: float = 0.0) -> float:
...
def apply_exit_fill(position: Dict, filled_amount: float) -> bool:
    ...
    filled_amount = positive_float(filled_amount)
    if current_amount <= 0 or filled_amount <= 0:
        return False
...
def configure_effective_leverage(exchange, symbols: List[str], leverage: int, journal=None) -> int:
    try:
        ...
        if leverage > 1:
            print("⚠️  Falling back to 1x spot sizing because leverage was not confirmed.")
        return 1
...
def place_entry_order(
    ...
    if use_limit_orders:
        ...
        try:
            ...
            filled_amount = get_confirmed_filled_amount(order_status)
            cancel_open_order(exchange, order_id, symbol, base_currency, "entry")
            if filled_amount > 0:
                _, average = extract_fill(order_status, filled_amount, limit_price)
                print(
                    f"[{base_currency}] ✅ Limit order partially filled: "
                    f"{filled_amount:.6f} {base_currency}"
                )
                return True, filled_amount, average
...
def place_exit_order(
    ...
    if use_limit_orders and not force_market:
        ...
        try:
            ...
            if order_status.get("status") == "closed":
                ...
                filled_amount = get_confirmed_filled_amount(order_status, amount)
    ...
    try:
        ...
        filled_amount = get_confirmed_filled_amount(order, amount)
        return True, filled_amount
    except Exception as exc:
        ...
        try:
            ...
            if not pos["in_position"]:
                ...
                if (
                    ...
                    if exit_executed:
                        closed_amount = min(
                            positive_float(exit_filled_amount, pos["position_amount"]),
                            pos["position_amount"],
                        )
                        journal.log_event(
                            "exit_executed",
                            ...
                            cost_usd=price * closed_amount,
                            profit_pct=profit_pct,
                            payload={
                                "estimated_pnl_usd": (price - entry_price) * closed_amount,
                                "peak_profit_pct": peak_profit_pct,
                                "drop_from_peak_pct": drop_from_peak_pct,
                            },
                        )
                        record_realized_pnl(daily_state, symbol, (price - entry_price) * closed_amount)
                        apply_exit_fill(pos, closed_amount)
                        continue
                    journal.log_event(
