#!/usr/bin/env python3
"""Regression tests for live order-state safety in main_multi_symbol.py."""
import ast
import pathlib
import types
import unittest
...
class FakeExchange:
    ...
    def create_market_buy_order(self, symbol, cost):
        ...
        return {"id": "market-buy", "filled": cost / self.ticker_price, "average": self.ticker_price}
    ...
    def create_market_sell_order(self, symbol, amount):
        ...
        return {"id": "market-sell", "filled": amount, "average": self.ticker_price}
...
class OrderExecutionSafetyTests(unittest.TestCase):
    ...
    def test_partially_filled_limit_entry_tracks_confirmed_fill(self):
        exchange = FakeExchange()
        exchange.next_status = {"status": "open", "filled": 0.25, "average": 99.9, "amount": 1.0}

        result = FUNCTIONS["place_entry_order"](
            exchange, "ETH/USD", "ETH", 1.0, 100.0, True, 0.001, True
        )

        self.assertEqual(result, (True, 0.25, 99.9))
        self.assertEqual(exchange.canceled, [("order-1", "ETH/USD")])
    ...
    def test_render_deploy_uses_execution_safety_gate(self):
        render_config = pathlib.Path(__file__).with_name("render.yaml").read_text()

        self.assertIn("startCommand: python app_entrypoint.py", render_config)
        self.assertIn("key: BOT_EXECUTE", render_config)
        self.assertIn("value: false", render_config)
        self.assertNotIn("python main.py --execute", render_config)
