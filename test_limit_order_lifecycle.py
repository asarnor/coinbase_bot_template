...
class LimitOrderLifecycleTests(unittest.TestCase):
    ...
    def test_limit_exit_create_failure_still_uses_market_fallback(self):
        _, place_exit_order = load_order_helpers()
        exchange = FakeExchange()
        exchange.limit_sell_error = RuntimeError("limit unavailable")

        executed = place_exit_order(exchange, "ETH/USD", "ETH", 0.25, "Profit-taking", True, 0.001, True)

        self.assertTrue(executed)
        self.assertIn(("create_market_sell_order", "ETH/USD", 0.25), exchange.calls)
